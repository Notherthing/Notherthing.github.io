# Yuchen Huang

**Email:** [ychuang@stu.ecnu.edu.cn](mailto:ychuang@stu.ecnu.edu.cn)

## Biography

Yuchen Huang is a Ph.D. candidate at the School of Data Science and Engineering, East China Normal University (ECNU), under the supervision of [Prof. Chuliang Weng](https://chuliangweng.github.io/). He received his bachelor's degree from the School of Computer Science and Technology, East China Normal University, in 2022.

## Research Interests

His research interests include hardware-software co-design for systems, with a recent focus on vector systems.

## Publications

\* indicates corresponding author.   ^ indicates equal contribution.

\- **Yuchen Huang**, Baiteng Ma, Erci Xu, and Chuliang Weng\*  
  ***Don't Surrender to Low QPS/\$: Fast and Cost-Efficient ANNS with TridentANN.***  
  *The 53rd International Symposium on Computer Architecture* (**ISCA 2026**), Full Paper (**CCF-A**).

\- **Yuchen Huang**, Baiteng Ma, Yiping Sun, Yang Shi, Xiao Chen, Xiaocheng Zhong, Zhiyong Wang, Yao Hu, Erci Xu\*, and Chuliang Weng  
  ***The Clustering Strikes Back: Building Cost-Effective and High-Performance ANNS at Scale with Helmsman.***  
  *The 20th USENIX Symposium on Operating Systems Design and Implementation* (**OSDI 2026**), Full Paper (**CCF-A**).

\- **Yuchen Huang^**, Baiteng Ma^, Yiping Sun, Yang Shi, Xiao Chen, Xiaocheng Zhong, Zhiyong Wang, Yao Hu, and Chuliang Weng\* 
  ***CCD-Level and Load-Aware Thread Orchestration for In-Memory Vector ANNS on Multi-Core CPUs.***  
  *The 42nd IEEE International Conference on Data Engineering* (**ICDE 2026**), Full Paper (**CCF-A**).

\- **Yuchen Huang**, Xiaopeng Fan, Song Yan, and Chuliang Weng\*  
  ***Neos: A NVMe-GPUs Direct Vector Service Buffer in User Space.***  
  *The 40th IEEE International Conference on Data Engineering* (**ICDE 2024**), Full Paper (**CCF-A**).

\- Xiaopeng Fan, Song Yan, **Yuchen Huang**, and Chuliang Weng\*  
  ***TEngine: A Native Distributed Table Storage Engine.***  
  *The 40th IEEE International Conference on Data Engineering* (**ICDE 2024**), Full Paper (**CCF-A**).

\- Xiaopeng Fan, Song Yan, **Yuchen Huang**, and Chuliang Weng\*  
  ***LUNAR: A Native Table Engine for Embedded Devices.***  
  *The 24th ACM SIGPLAN/SIGBED International Conference on Languages, Compilers, and Tools for Embedded Systems* (**LCTES 2023**, co-located with **PLDI 2023**), Full Paper (**CCF-B**).

## Education

**2025–Present (Expected: Dec. 2026)**  *Ph.D. Candidate*  
East China Normal University, School of Data Science and Engineering

**2022–2025**  *Master*  
East China Normal University, School of Data Science and Engineering

**2018–2022**  *Bachelor*  
East China Normal University, School of Computer Science and Technology